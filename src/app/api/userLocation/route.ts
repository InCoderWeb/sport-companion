...
export async function POST(request: NextRequest) {
	try {
		...
		const { userId, lat, lng, updatedAt } = body;
		...
		if(user !== null){
			...
			new UsersLocation({ userId, lat, lng, createdAt : new Date(), updatedAt }).save();
			return NextResponse.json(
				{
					status: true,
...
export async function GET() {
	try {
		const usersLocation = await UsersLocation.find({ userId: "0" });
		if (usersLocation) {
			return NextResponse.json(
				{
					status: true,
